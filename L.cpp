...
void upd(int rt, int l, int r, int ql, int qr, ll v, ll tx){

    if (r < ql || l > qr) return;
    //cout << l << ' ' << r << ' ' << ql << ' ' << qr << endl;
    if (l >= ql && r <= qr && tree[rt].isMul[isp[v]] == 1){
        tree[rt].sum = ((tree[rt].sum)%MOD*tx)%MOD;
        tree[rt].mul = ((tree[rt].mul)%MOD*tx)%MOD;
        return;
    }
    if (l >= ql && r <= qr && tree[rt].isMul[isp[v]] == 0){
        tree[rt].sum = ((tree[rt].sum*(v-1))%MOD*(tx/v))%MOD;
        tree[rt].mul = ((tree[rt].mul*(v-1))%MOD*(tx/v))%MOD;
    ...
    int mid = (l+r)/2;
    upd(rt<<1, l, mid, ql, qr, v, tx);
    upd(rt<<1|1, mid+1, r, ql, qr, v, tx);
...
int main(){
    ...
    while(m--){
        ...
        //cout << "---" << endl;
        //cout << tree[1].sum << ' ' << tree[1].mul << endl;
        ...
        if (op == 0){
            ...
            for (int i = 1;i <= cnt; ++i){
                ...
                if (w%pri[i] == 0){
                    int tx = 1;
                    while(w%pri[i] == 0) {w /= pri[i]; tx *= pri[i];}
                    //cout << pri[i] << endl;
                    //cout << l << ' ' << r << ' ' << pri[i] << endl;
                    upd(1, 1, n, l, r, pri[i], tx);
                }
            }
        }
        else{
